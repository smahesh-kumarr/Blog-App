...
const HomePage = () => {
  ...
  const fetchAllBlogs = async () => {
    try {
      const response = await fetch('http://localhost:5000/blogs/recent');
      if (response.ok) {
  ...
  const fetchUserBlogs = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/blogs/user/${encodeURIComponent(userInfo.email)}`);
      if (response.ok) {
        const data = await response.json();
        setMyBlogs(data);
      }
  ...
  const handleUpdateBlog = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/blogs/${editingBlogId}`, {
        method: 'PUT',
        body: formData
      });
  ...
  const handleDeleteSelected = async () => {
    ...
    if (window.confirm('Are you sure you want to delete the selected blogs?')) {
      try {
        const response = await fetch('http://localhost:5000/blogs/delete-multiple', {
          method: 'DELETE',
          headers: {
